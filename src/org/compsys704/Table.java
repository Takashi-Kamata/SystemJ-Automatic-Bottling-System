...
//		this.setPreferredSize(new Dimension(200, 300));
		panel = new Canvas_Table();
		panel.setPreferredSize(new Dimension(232,168));
		panel.setBackground(Color.WHITE);
		JButton enable = new JButton("enable");
		enable.addActionListener(new SignalClient(Ports.PORT_LOADER_PLANT, Ports.TABLE_ENABLE_SIGNAL));
		JButton request = new JButton("request");
		request.addActionListener(new SignalClient(Ports.TABLE_PORT_LOADER_CONTROLLER, Ports.TABLE_REQUEST_SIGNAL));
...
//		this.add(pan3,c);
	...
	public static void main(String[] args) {
		...
		SignalServer<LoaderVizWorker> server = new SignalServer<LoaderVizWorker>(Ports.TABLE_PORT_LOADER_VIZ, LoaderVizWorker.class);
